...
def _bpt1 = TruncatedBP(Period, Bandwidth, Length, Fundamental(Price, Symbol1)).BPT;
plot BPT1 = if Normalize then AutoGainCtrl(1, Period, Bandwidth, Length, Fundamental(Price, Symbol1)).Normalized else _bpt1;
BPT1.SetDefaultColor(GetColor(2));
AddVerticalLine(BPT1[1] <= BPT1[2] && BPT1 > BPT1[1], "BPT1U", GetColor(2), Curve.Firm);
AddVerticalLine(BPT1[1] >= BPT1[2] && BPT1 < BPT1[1], "BPT1D", GetColor(2), Curve.FIRM);

AddLabel(1, "Value", GetColor(2));

def _bpt2 = TruncatedBP(Period, Bandwidth, Length, Fundamental(Price, Symbol2)).BPT;
plot BPT2 = if Normalize then AutoGainCtrl(1, Period, Bandwidth, Length, Fundamental(Price, Symbol2)).Normalized else _bpt2;
